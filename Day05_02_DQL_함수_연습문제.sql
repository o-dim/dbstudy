...
FROM EMPLOYEES
WHERE JOB_ID LIKE 'IT_PROG' OR JOB_ID LIKE 'ST_MAN';
...
SELECT FIRST_NAME, LAST_NAME, COMMISSION_PCT * SALARY AS 실제커미션
FROM EMPLOYEES
...
SELECT FIRST_NAME, 
       ...
       CASE 
            ...
            WHEN PHONE_NUMBER LIKE '630%' THEN 'SOUTH'
...
SELECT  FIRST_NAME,
        LAST_NAME,
        HIRE_DATE,
        (TO_DATE(SYSDATE, 'YY/MM/DD') - TO_DATE(HIRE_DATE, 'YY/MM/DD')) AS WORKINGMONTH,
        CASE
            WHEN WORKINGMONTH >= 240 THEN '퇴직금정산대상'
            ELSE ' '
        END
FROM EMPLOYEES;
                
-- 16. EMPLOYEES 테이블에서 SALARY 평균이 10000 이상인 부서의 DEPARTMENT_ID와 SALARY 평균을 조회하시오. 평균은 정수로 내림처리하시오.
SELECT DEPARTMENT_ID, AVG(SALARY) AS 월급평균
FROM EMPLOYEES
GROUP BY DEPARTMENT_ID
HAVING AVG(SALARY) >= 10000;

-- 17. EMPLOYEES 테이블에서 DEPARTMENT_ID와 JOB_ID가 모두 같은 사원들을 그룹화하여 각 그룹의 사원수를 조회하시오. DEPARTMENT_ID가 NULL인 사원은 제외하시오.
SELECT NVL(DEPARTMENT_ID, 0), JOB_ID, COUNT(*) AS 그룹사원수,
       CASE
            WHEN DEPARTMENT_ID = JOB_ID THEN ' '
        END
